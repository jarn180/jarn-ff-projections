...
- **Web App**: Notion-inspired React UI (Tailwind CSS, Framer Motion, Lucide icons), responsive on mobile and desktop
...
- Sortable, filterable projections table (desktop) that collapses into a card list on mobile
- Search functionality to find specific players
- Format selector (PPR, Half-PPR, Standard, or All) and position tabs (QB/RB/WR/TE/FLEX)
- Detailed player breakdowns in an animated modal
- Start/Sit optimizer with Sleeper league integration
- Clean, Notion-inspired interface with light/dark theming

#### Frontend development

The UI lives in `frontend/` (React + Vite + Tailwind CSS + Framer Motion + Lucide icons) and is served by Flask as a single-page app. Flask reads the **built** assets from `static/app/`, which are committed to the repo since there's no separate frontend build step in deployment.

To work on the UI:
```bash
cd frontend
npm install
npm run dev      # dev server with API requests proxied to Flask on :5000
```

When you're done, rebuild the production bundle so Flask serves your changes:
```bash
cd frontend
...
├── templates/
│   └── index.html             # SPA shell template (loads static/app bundle)
├── static/
│   └── app/                   # Built React frontend (committed, generated by `npm run build`)
├── frontend/                  # React + Vite + Tailwind + Framer Motion source
│   └── src/
