...
document.getElementById("connectBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    if (data.success) {
      ...
      showMessage(
        messageDiv,
        `Connected as ${data.user.display_name || username}`,
        "success"
      );
...
async function loadLeagues(userId) {
  ...
  leaguesList.innerHTML =
    '<div class="loading-message"><div class="spinner-large"></div><p>Loading your leagues...</p></div>';
  leagueSection.style.display = "block";

  try {
    // Try 2026 season first (2026-2027 NFL season)
    let response = await fetch(
      `/api/sleeper/user/${userId}/leagues?season=2026`
    );
    let data = await response.json();

    // If no leagues found, try 2025
    if (data.success && data.leagues.length === 0) {
      response = await fetch(`/api/sleeper/user/${userId}/leagues?season=2025`);
    ...
    if (data.success && data.leagues.length > 0) {
      ...
      leaguesList.innerHTML =
        '<div class="no-results">No leagues found for 2025-2026 season. Please check your username.</div>';
    }
  } catch (error) {
    leaguesList.innerHTML =
  ...
  const html = `
        <div class="leagues-grid">
            ${leagues
              .map(
                (league) => `
                <div class="league-card" onclick="selectLeague('${
                  league.league_id
                }', '${league.name}', '${league.scoring_settings?.rec || 0}')">
                    <h3>${league.name}</h3>
                    <div class="league-details">
                        <span class="badge">${league.total_rosters} Teams</span>
                        <span class="badge">${getScoringType(
...
async function selectLeague(leagueId, leagueName, recPoints) {
  ...
  document.getElementById("leagueScoring").textContent = getScoringType({
    rec: recPoints,
  });
  ...
  try {
    const response = await fetch(
      `/api/sleeper/league/${leagueId}/roster/${currentUserId}`
    );
    const data = await response.json();
    ...
    if (data.success) {
      ...
      document.getElementById("userRecord").textContent = `${wins}-${losses}${
        ties > 0 ? `-${ties}` : ""
      }`;
    }
  } catch (error) {
...
document.getElementById("optimizeBtn").addEventListener("click", async () => {
  ...
  resultsDiv.innerHTML =
    '<div class="loading-message"><div class="spinner-large"></div><p>Analyzing your roster...</p></div>';
  ...
  try {
    ...
    resultsDiv.innerHTML =
      '<div class="error-message">Failed to optimize lineup. Please try again.</div>';
  } finally {
    btn.disabled = false;
  ...
  const html = `
        <div class="optimization-results">
            ...
            <div class="stats-bar" style="margin-bottom: 30px;">
                <div class="stat-item">
                    ...
                    <span class="stat-value" style="font-size: 1.5rem;">${
                      optimization.total_projection
                    }</span>
                </div>
                <div class="stat-item">
                    <span class="stat-label">Week:</span>
                    <span class="stat-value">${
                      optimization.current_week || "TBD"
                    }</span>
                </div>
                <div class="stat-item">
                    <span class="stat-label">Format:</span>
                    <span class="stat-value">${
                      optimization.scoring_format
                    }</span>
                </div>
            </div>
                            ...
                            <div class="recommendation-card">
                                ...
                                <div class="rec-swap">
                                    <div class="rec-action">
                                        ...
                                        <span class="player-proj">${rec.bench_projection.toFixed(
                                          1
                                        )} pts</span>
                                    </div>
                                    ...
                                    <div class="rec-action sit">
                                        ...
                                        <span class="player-proj">${rec.starter_projection.toFixed(
                                          1
                                        )} pts</span>
                                    </div>
                                </div>
            ...
            <div class="lineup-section">
                ...
                ${Object.entries(optimization.starters)
                                        ...
                                        <div class="player-info">
                                            ...
                                            ${
                                              player.injury_status
                                                ? `<span class="injury-badge">${player.injury_status}</span>`
                                                : ""
                                            }
                                        </div>
                                        ${
                                          player.no_lines
                                            ...
                                            : `<span class="player-projection">${player.projection.toFixed(
                                                1
                                              )} pts</span>`
                                        }
                                    </div>
